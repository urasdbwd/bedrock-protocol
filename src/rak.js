...
const { Socks5UdpProxy } = require('./socks5-udp-proxy')
...
class RakNativeClient extends EventEmitter {
  constructor (options, client) {
    ...
    this.raknetClientPort = null
    this.proxyMessageHandler = null

    const protocolVersion = client?.versionGreaterThanOrEqualTo('1.19.30') ? 11 : 10

    if (options.proxy) {
      console.log('[RakNet] 🔒 Proxy mode enabled')
      this.useProxy = true
      this.setupProxyRelay(protocolVersion)
    } else {
      this.useProxy = false
      this.raknet = new Client(options.host, options.port, { protocolVersion })
      this.setupRaknetEvents()
    }
  }

  async setupProxyRelay(protocolVersion) {
    this.localRelay = dgram.createSocket('udp4')
    
    await new Promise((resolve) => {
      this.localRelay.bind(0, '127.0.0.1', () => {
        this.localRelayPort = this.localRelay.address().port
        this.raknet = new Client('127.0.0.1', this.localRelayPort, { protocolVersion })
        this.setupRaknetEvents()
        resolve()
      })
    })

    this.localRelay.on('message', async (msg, rinfo) => {
      if (!this.proxy) return
      
      if (!this.raknetClientPort) {
        this.raknetClientPort = rinfo.port
      }
      
      this.proxy.sendPacket(msg)
    })
  }
  ...
  setupProxyEventHandlers(udpSocket) {
    this.proxyMessageHandler = (buffer, rinfo) => {
      if (!this.raknetClientPort) return
      
      this.localRelay.send(buffer, this.raknetClientPort, '127.0.0.1', (err) => {
        if (err) console.error('[RakNet] ❌ Error forwarding:', err.message)
      })
    }

    udpSocket.on('proxyMessage', this.proxyMessageHandler)

    this.proxy.on('tcp-disconnected', (reason) => {
      console.warn('[RakNet] ⚠️  Proxy TCP disconnected:', reason)
    })

    this.proxy.on('tcp-reconnected', () => {
      console.log('[RakNet] ✅ Proxy TCP reconnected')
    })
  }

  async ping (timeout = 1000) {
    if (this.useProxy) {
      console.warn('[RakNet] ⚠️  Ping through proxy not fully supported')
    }
  ...
  async connect () {
    if (this.useProxy) {
      console.log('[RakNet] 🔗 Connecting through proxy...')
      
      this.proxy = new Socks5UdpProxy({
        proxyHost: this.options.proxy.host,
        proxyPort: this.options.proxy.port,
        proxyUsername: this.options.proxy.username,
        proxyPassword: this.options.proxy.password,
        targetHost: this.options.host,
        targetPort: this.options.port
      })

      try {
        const { udpSocket } = await this.proxy.connect()
        console.log('[RakNet] ✅ Proxy connected')
        
        this.setupProxyEventHandlers(udpSocket)
        this.raknet.connect()
      } catch (err) {
        console.error('[RakNet] ❌ Proxy connection failed:', err.message)
        throw err
      }
    } else {
      this.raknet.connect()
  ...
  close () {
    ...
    if (this.useProxy) {
      ...
      if (this.localRelay) this.localRelay.close()
    }
...
class RakJsClient extends EventEmitter {
  constructor (options = {}) {
    ...
    if (options.proxy) {
      throw new Error('Proxy not supported with jsp-raknet backend. Use raknetBackend: "raknet-native"')
    }
